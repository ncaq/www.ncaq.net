...
```json
{
  ...
  "mounts": ["source=nix-store-${devcontainerId},target=/nix,type=volume"],
  ...
  "customizations": {
    "vscode": {
      ...
      "extensions": ["jnoortheen.nix-ide", "mkhl.direnv"]
    }
  },
  "remoteUser": "vscode"
}
...
substituters = https://cache.nixos.org https://nix-community.cachix.org https://example.cachix.org
trusted-public-keys = cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY= nix-community.cachix.org-1:mB9FSh9qf2dCimDSUo8Zy7bkq5CX+/rkCWyvRCYg3Fs= example.cachix.org-1:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX=
NIXCONF
...
## 初回起動時にnixのボリュームが権限問題で書き込み不可になる

自分は最初の頃に試行錯誤していた時にボリュームが作られていたので気が付きませんでしたが、
ゼロから起動すると`/nix`ボリュームがroot所有になって以下のエラーになります。

```text
performing a single-user installation of Nix...
/tmp/nix-binary-tarball-unpack.JwXpPwrfT8/unpack/nix-2.31.2-x86_64-linux/install: directory /nix exists, but is not writable by you. This could indicate that another user has already performed a single-user installation of Nix on this system. If you wish to enable multi-user support see https://nix.dev/manual/nix/stable/installation/multi-user.html. If you wish to continue with a single-user install for vscode please run 'chown -R vscode /nix' as root.
```

なので`postCreateCommand`の最初に権限修正を入れました。

ついでにキャッシュ設定についても、
nixをインストールする分には公式のキャッシュサーバがあるので問題ないのですが、
プライベートキャッシュにアクセスできないというエラーが出るのが心理的に不安なため、
nixのインストールより先に済ましておくことにします。
キャッシュサーバなどの設定はnix環境が提供するコマンドは不要なため十分可能です。
