...
nixpkgsのパッケージ情報やNixOSのオプションやHome Managerのオプションなどの、
Nixに関する情報をMCP(Model Context Protocol)経由で提供するサーバです。
AIアシスタントがNixOS関連のパッケージ名や設定を検索することが、
不正確になりやすいWebFetch経由ではなく容易になります。
...
流石に知らない人からのリクエストを自分が実装したわけではない知らないプログラムで処理するのは怖いので。
...
に書いたように、
その時は[NixOS Containers](https://nixos.wiki/wiki/NixOS_Containers)でサービスを分離していました。
...
今回はあまり知らないコードが不特定なリクエストで処理するので、
仮想マシンの方が安心です。
...
まずmicrovm.nixで使う仮想マシンはフットプリントとかを考えて統一したいと思うので、
