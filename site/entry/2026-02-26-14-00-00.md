...
AndroidデバイスをSSHクライアントとして使う場合に、
最良の選択肢を探した結果、
[Nix-on-Droid](https://github.com/nix-community/nix-on-droid)をセットアップしました。

# 背景

## AIエージェントによる状況の変化

[Claude Code](https://claude.ai/code)のようなAIエージェントが実用的になったことで、
小さい画面でのプログラミング作業が現実的になってきました。
これまではスマートフォンでコードを書くのは効率が悪すぎましたが、
AIエージェントに作業を委譲できるなら画面サイズの問題は大きく緩和されます。
...
普段のサーバに直接SSH接続してClaude Codeを使う方が圧倒的に快適です。
...
systemdサービスに依存している設定はTermux環境で無効化が必要です。
