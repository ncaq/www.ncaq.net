---
title: Nix-on-DroidをセットアップしてAndroidの最良のSSHクライアントを手に入れた
---
...
gpg-agentを`SSH_AUTH_SOCK`に設定するだけで完全に動作しました。
...
とはいえNix環境がAndroidで使えること自体は便利です。
端末を買い替えた際も`install.sh`一発で環境が再現できるのは継続して有用です。
