...
このリポジトリは静的サイトを生成しているため、
ドキュメントに見えるものが`docs`ではなくて`feat`のtypeになることがあります。

特に`site/entry`以下にmarkdown形式で記事を追加する場合、
タイトルがそのまま`feat`になることがあります。

例えば以下のようなタイトル設定の記事を追加した場合、
...
```
feat: NixOSの自宅サーバにCloudflare TunnelでHTTPリクエストを通す
```
