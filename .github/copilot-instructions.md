...
リンクは内容がシンプルな場合は`[リンクにしたいシンプルな文字列](URL)`の形式で貼ってください。

リンクを単独で成立させたい時は`[完全なページタイトル](URL)`の形式で貼ってください。

過去の記事と関係する場合は積極的にリンクを貼ってください。

外部のものに言及する場合も積極的にリンクを貼ってください。

プロジェクトやソフトウェアに言及するときはなるべくリンクを貼ってください。

問題を表すissueへのリンクや実装PRへのリンクも積極的に貼ってください。

リンクを貼る時は本当にリンク先が存在しているか確認してください。
...
サイトの構築プログラムは`src`ディレクトリ以下に記述されています。
...
記事ファイルが正常に作成されるかはリポジトリルートにあるbuildスクリプトを実行して確認できます。
