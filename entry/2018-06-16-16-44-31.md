...
`if (x)`を使いたくない時でも面倒なtypeofは使わないで素直に`x === undefiend`を使いましょう.
...
そういう時,MDNでは`in`を使ったグローバルスコープのルックアップを参照を推奨していますが,undefiendやnull値が入ったプロパティを弾けないので私は推奨しません.
