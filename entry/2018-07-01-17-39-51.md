...
* GCがある分その分遅い
* 私がC++の経験の方が多いのでD言語のライブラリをあまり把握してない
* 日本語資料があまりないのでライブラリの解説は英語を読む必要がある

## D言語がHaskellに対して勝る点

* 破壊的操作を容易に行えるためメモ化とかを頻繁に行う競技プログラミングでは面倒が少ない
* リストじゃなくて配列なので速いことが多い

## D言語がRustに対して勝る点

* 競技プログラミングみたいな書き捨てのコードでライフタイムとか気にしたくない,保守必要なやつならともかく
* GCあるから標準ライブラリのAPIがシンプルに収まっている
...
自分で気にしている暇はありませんが,自分のコードを見返すこと自体はそこそこあります.
スタイルが整っていると,バグも発見しやすくなります.

当然D言語でも[dfmt](https://github.com/dlang-community/dfmt)
というパッケージを使ってフォーマットが出来ます.

また,Emacsにもmelpaにdfmtパッケージがあります.
...
全体のインデントは保存時に行いますが,編集中もなるべくdfmtのインデントレベルに合わせたいですね.

当初は`indent_size`は2にして,`indent_style`はotbsにしようと思っていました.
しかし,[The D Style - D Programming Language](https://dlang.org/dstyle.html)
を見ると,オールマンスタイルとインデントサイズ4を推奨していたので,
郷に従うことにしました.
...
しかし,本当にD言語で競技プログラミングを本格的に行うかわからないので,
今回は導入を見送ることにしました.
...
速度が重視されるものですので,当然ながらLDCを使いたい所ですが,
...
コピーして依存関係を修正しましたが,
当然ですがビルドエラーが起きますね.
どうやらllvm 3を要求していて,llvm 6では動かないようです.
...
DMDなら2.070という同じものが使えるので,ひとまずDMDで戦うことにしましょう.
